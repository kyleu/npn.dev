...
(function(d){var b=function(){function e(){this.storage=Object.create(null)}e.prototype.get=function(f){return this.storage[f]};e.prototype.set=function(f,a){return this.storage[f]=a};e.prototype.del=function(f){delete this.storage[f]};return e}();d.Map=b})(map||(map={}));var nav;
...
(function(d){d.renderBody=function(b,e){if(!e)return JSX("div",null,"No body");switch(e.type){case "json":return e=Prism.highlight(JSON.stringify(e.config.msg,null,2),Prism.languages.javascript,"javascript"),JSX("div",null,JSX("em",null,"JSON"),JSX("div",{class:"uk-margin-top"},JSX("pre",{class:"language-html","data-start":"0",style:"white-space:pre-wrap;"},JSX("code",{dangerouslySetInnerHTML:{__html:e}}))));case "html":return b=request.baseURL(b),e=Prism.highlight(e.config.content,Prism.languages.html,
"html"),JSX("div",{class:"html-body"},JSX("span",{class:"base-url hidden"},b),JSX("span",{class:"preview-link right"},"(",JSX("a",{class:style.linkColor,href:"",onclick:"rbody.renderHTMLPreview(this);return false"},"preview"),")"),JSX("span",{class:"text-link right hidden"},"(",JSX("a",{class:style.linkColor,href:"",onclick:"rbody.renderHTMLText(this);return false"},"text"),")"),JSX("em",null,"HTML"),JSX("div",{class:"text-content prism uk-margin-top",style:"overflow: auto; max-height: 720px;"},JSX("pre",
{class:"language-html","data-start":"0",style:"white-space:pre-wrap;"},JSX("code",{dangerouslySetInnerHTML:{__html:e}}))),JSX("div",{class:"preview-content uk-margin-top hidden",style:"overflow: auto; max-height: 720px; border: 1px solid #666;"}));case "image":return e=e.config,JSX("img",{alt:"response image",src:"data:"+e.type+";base64,"+e.content});case "raw":return e=e.config,JSX("div",null,JSX("em",null,e.type?e.type:"Unknown Type"),JSX("pre",null,json.str(e)));case "error":return e=e.config,
JSX("div",null,JSX("em",null,"Error"),JSX("pre",null,e.message));default:return JSX("div",null,"unhandled body type [",e.type,"]")}}})(rbody||(rbody={}));
...
"Requests"),JSX("form",{onsubmit:"collection.addRequestURL('"+f.key+"');return false;"},JSX("div",{class:"uk-margin-top uk-inline uk-width-expand"},JSX("button",{class:"uk-form-icon uk-form-icon-flip",type:"submit",title:"add request","uk-icon":"icon: plus"}),JSX("input",{id:"coll-request-add-url",class:"uk-input",placeholder:"add a request by url","data-lpignore":"true"}))),JSX("div",{id:"request-list",class:"uk-margin-top"},e(f.key,a))))};d.renderNotFound=function(f){return JSX("div",{class:"uk-card uk-card-body uk-card-default"},
JSX("div",{class:"right"},JSX("a",{class:"theme uk-icon","data-uk-icon":"close",href:"",onclick:"nav.pop();return false;",title:"close collection"})),JSX("h3",{class:"uk-card-title"},JSX("span",{class:"nav-icon-h3","data-uk-icon":"icon: album"}),f),JSX("p",null,"Collection [",f,"] not found"))};d.addFromInput=function(){var f=dom.req("#coll-add-input"),a=f.value.trim();a&&0<a.length&&(f.value="",socket.send({svc:services.collection.key,cmd:command.client.addCollection,param:a}),log.info("adding request ["+
a+"]"))};d.addRequestURL=function(f){var a=dom.req("#coll-request-add-url"),c=a.value.trim();c&&0<c.length&&(a.value="",socket.send({svc:services.collection.key,cmd:command.client.addRequestURL,param:{coll:f,url:c}}),log.info("adding request ["+c+"]"))}})(collection||(collection={}));
(function(d){d.onCollectionMessage=function(b,e){var f;switch(b){case command.server.collections:d.cache.collections=group.sort(e,function(c){return c.key});log.info("processing ["+d.cache.collections.length+"] collections");d.renderCollections(d.cache.collections);break;case command.server.collectionDetail:log.info("processing ["+((null===(f=e.requests)||void 0===f?void 0:f.length)||0)+"] requests for collection ["+e.key+"]");e.collection?d.cache.updateCollection(e.collection):d.cache.collections=
d.cache.collections.filter(function(c){return c.key!==a});request.cache.setCollectionRequests(e.key,e.collection,e.requests||[]);d.renderCollections(d.cache.collections);break;case command.server.collectionAdded:log.info("processing new collection ["+e.active+"]");d.cache.collections=e.collections;nav.navigate("/c/"+e.active);break;case command.server.collectionDeleted:var a=e;log.info("processing deleted collection ["+a+"]");d.cache.collections=d.cache.collections.filter(function(c){return c.key!==
a});d.renderCollections(d.cache.collections);nav.navigate("/");break;default:console.warn("unhandled collection command ["+b+"]")}}})(collection||(collection={}));var header;
...
(function(d){var b=function(){function e(){this.summaries=new map.Map;this.requests=new map.Map;this.extra=[]}e.prototype.setCollectionRequests=function(f,a,c){if(a){if(this.summaries.set(f,c),f===collection.cache.active)for(dom.setContent("#collection-panel",collection.renderCollection(a,c)),a=0;a<c.length;a++){var g=c[a];this.active===g.key&&(d.renderActiveRequest(f),this.action&&d.renderAction(f,g.key,this.action,this.extra))}}else this.summaries.del(f),f===collection.cache.active&&dom.setContent("#collection-panel",
collection.renderNotFound(f))};e.prototype.setActiveRequest=function(f,a){f&&this.active!==a&&((this.active=a)&&d.renderActiveRequest(f),collection.renderCollections(collection.cache.collections))};e.prototype.setActiveAction=function(f,a,c){f&&(this.extra.length===c.length&&this.extra.every(function(g,h){return g===c[h]}),this.active&&(this.action=a,this.extra=c,d.renderAction(f,this.active,this.action,this.extra)))};e.prototype.updateRequest=function(f,a){var c=this.requests.get(f);c=group.update(c,
a,function(g){return g.key});this.requests.set(f,c);c=this.summaries.get(f);c=c.map(function(g){return g.key==a.key?d.toSummary(a,0):g});this.summaries.set(f,c);collection.cache.active===f&&collection.renderCollection(collection.cache.getActiveCollection(),c)};e.prototype.removeRequest=function(f,a){var c=this.requests.get(f);c=group.remove(c,a,function(g){return g.key});this.requests.set(f,c);c=this.summaries.get(f);c=c.filter(function(g){return g.key!==a});this.summaries.set(f,c);this.active===
a&&d.cache.setActiveRequest(f,void 0);collection.cache.active===f&&collection.renderCollection(collection.cache.getActiveCollection(),c)};return e}();d.cache=new b})(request||(request={}));
