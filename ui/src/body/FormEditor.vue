<template>
  <div v-if="(!qp) || qp.length === 0" class="mt">
    No form fields defined, why not <a href="" @click.prevent="addParam()">add one</a>?
  </div>
...
export default class FormEditor extends Vue {
  get qp(): QueryParam[] | undefined {
    return bodyConfigRef.value?.formContent;
  }

  set qp(x: QueryParam[] | undefined) {
    if(bodyConfigRef.value) {
      bodyConfigRef.value.formContent = x || [];
    }
  }
