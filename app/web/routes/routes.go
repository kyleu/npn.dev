...
func BuildRouter(app *config.AppInfo) (*mux.Router, error) {
	...
	r.Path(p(util.KeySchema, "{key}", util.KeyService, "{s}")).Methods(http.MethodGet).Handler(addContext(r, app, http.HandlerFunc(controllers.SchemaServiceDetail))).Name(n(util.KeySchema, util.KeyService))
