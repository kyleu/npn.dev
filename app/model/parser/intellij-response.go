...
func parseTable(r *IntelliJResponse, ij *ijTable) error {
	...
	err := r.Schema.AddModel(&schema.Model{Key: ij.Name})
	if err != nil {
		return err
	}
	err = r.resolveChildren(ij.ID)
...
func parseColumn(r *IntelliJResponse, ij *ijColumn) error {
	table := r.Schema.Models.Get(r.state["table"])
	if table == nil {
		return errors.New("no table found with key [" + r.state["table"] + "]")
	}
	t := schematypes.Wrap(schematypes.Unknown{X: ij.DataType})
	if ij.NotNull == 0 {
		t = schematypes.Wrap(schematypes.Option{T: t})
	}
